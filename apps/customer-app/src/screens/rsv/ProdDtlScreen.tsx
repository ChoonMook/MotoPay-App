...
import { TINT_POSITION_LABELS } from "./rsvTypes";
...
export interface ProdDtlInfo {
  ...
  brand: string | null; // 제품 테이블(TINT_PRODUCTS)에서 매칭된 브랜드 — 매칭 안 되면 null
  spec: string | null; // 제품 테이블에서 매칭된 스펙 요약 — 매칭 안 되면 null
      ...
      <div className="absolute inset-0 flex flex-col" style={{ animation: "mp-screen .32s ease" }}>
        ...
        <div className="mp-scroll flex-1 overflow-y-auto px-5 pt-[18px] pb-6">
          ...
          {info.positions.length > 0 && (
            <>
              <div className="mx-0.5 mt-[22px] mb-2.5 text-[13px] font-extrabold text-gray-900">부위별 농도</div>
              <div className="rounded-2xl border border-gray-200 bg-white px-4 shadow-sm">
                {info.positions.map((p, i) => (
                  <div
                    key={p.position}
                    className={`flex items-center justify-between gap-3 py-3.5 ${i < info.positions.length - 1 ? "border-b border-gray-100" : ""}`}
                  >
                    <span className="text-xs text-gray-500">{TINT_POSITION_LABELS[p.position] ?? p.position}</span>
                    <span className="text-right text-[13px] font-bold text-gray-900">{p.level}%</span>
                  </div>
                ))}
              </div>
            </>
