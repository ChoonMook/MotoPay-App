...
export class ProductPositionOptionGroupDto {
  @ApiProperty({ example: 'FRONT', description: '시공 부위 -> CommonCodeDetail(code=BID_TINT_POSITION)' })
  @IsString()
  @MinLength(1)
  position: string;

  @ApiProperty({ example: ['5', '15', '30'], description: '이 부위에서 선택 가능한 농도 목록 -> CommonCodeDetail(code=VLT)' })
  @IsArray()
  @IsString({ each: true })
  levels: string[];
}
...
export class SetProductPositionOptionsDto {
  ...
  @ApiProperty({ type: [ProductPositionOptionGroupDto], description: '부위별 선택 가능 농도 설정' })
  @IsArray()
