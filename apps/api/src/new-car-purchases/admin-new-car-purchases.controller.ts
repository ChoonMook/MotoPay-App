...
export class AdminNewCarPurchasesController {
  constructor(
    private readonly newCarPurchasesService: NewCarPurchasesService,
  ) {}

  @Get()
  @ApiOperation({
    summary: '등록 내역 조회(DL-NCPK-04) — 고객명·VIN 검색, 확정상태 필터',
  })
  list(
  ...
  create(
    @Body() dto: CreateNewCarPurchaseDto,
  ...
  @ApiOperation({
    summary:
      '엑셀 일괄업로드로 파싱한 행 목록 일괄 등록(DL-NCPK-03) — 행별 개별 성공/실패 반환',
  })
  bulkCreate(
    @Body() dto: BulkCreateNewCarPurchasesDto,
    @CurrentAdmin() me: SafeAdminAccount,
  ) {
  ...
  @ApiOperation({
    summary:
      '구매 내역 수정 — 확정된 건은 관리자(accountType=ADMIN)만 수정 가능',
  })
  update(
