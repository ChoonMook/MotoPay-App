...
export class PartnerBidRequestsController {
  ...
  @Get()
  @ApiOperation({ summary: '내 업체로 입찰의뢰가 온 예약시공 요청 목록(입찰함)' })
  ...
  @ApiOperation({ summary: '입찰 제출(항목별 견적) — 고객이 업체를 선택하기 전까지는 재제출로 수정 가능' })
  async submitOffer(
    @CurrentPartnerUser() partnerUser: SafePartnerUser,
    @Param('requestNo') requestNo: string,
    @Body() dto: SubmitBidOfferDto,
  ) {
    await this.bidRequestsService.submitOffer(partnerUser.shopCode, requestNo, dto);
    return { success: true };
  }

  @Post(':requestNo/plans')
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: '추천안 제출(상품·부위/농도·시공시각·사유) — 고객이 업체를 선택하기 전까지는 재제출로 수정 가능' })
  async submitPlan(
    @CurrentPartnerUser() partnerUser: SafePartnerUser,
    @Param('requestNo') requestNo: string,
    @Body() dto: SubmitBidPlanDto,
  ) {
    await this.bidRequestsService.submitPlan(partnerUser.shopCode, requestNo, dto);
    return { success: true };
  }
}
