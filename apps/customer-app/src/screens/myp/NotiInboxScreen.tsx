...
export default function NotiInboxScreen({ onBack, onOpenSettings, notiRead, onMarkRead }: NotiInboxScreenProps) {
  return (
    <div className="absolute inset-0 flex flex-col" style={{ animation: "mp-screen .32s ease" }}>
      ...
      <div className="mp-scroll flex-1 overflow-y-auto px-5 pt-2 pb-6">
        <div className="flex flex-col">
          {NOTI_DEFS.map((n, i) => {
            const unread = !notiRead[n.id];
            return (
              <div
                key={n.id}
                onClick={() => onMarkRead(n.id)}
                className={`flex cursor-pointer gap-2.5 py-3.5 ${i < NOTI_DEFS.length - 1 ? "border-b border-gray-100" : ""}`}
              >
