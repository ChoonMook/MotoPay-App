...
import { notifyApiBusyDelta } from "../lib/parentBridge";
...
async function handle<T>(response: Response): Promise<T> {
  ...
  // DELETE 등 일부 응답은 본문이 비어있어(204 또는 200+빈 바디) response.json()이 예외를 던짐 - 빈 바디면 undefined 반환
...
function refreshAccessToken(): Promise<boolean> {
  if (!refreshInFlight) {
    refreshInFlight = (async () => {
      ...
      try {
        const response = await rawFetch("/admin-auth/refresh", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
