...
// 네이티브(customer-mobile — 파트너센터도 같은 웹뷰 셸로 로드됨)가 하드웨어 백버튼을 누르면
// window.__motoConsumeBack()을 호출해 현재 열려 있는 화면·시트가 자체 뒤로가기를 처리했는지 확인함
// (처리 못 하면 네이티브가 앱을 종료함) — apps/customer-app/src/native/backHandler.ts와 동일 구현
